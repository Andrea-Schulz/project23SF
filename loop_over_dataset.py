...
## Set parameters and perform initializations:
    # Select Waymo Open Dataset file and frame numbers via: data_filename, show_only_frames
    # exec_detection options: 'bev_from_pcl', 'detect_objects', 'validate_object_labels', 'measure_detection_performance'; options not in the list will be loaded from file
...
if exercise == 'ID_S1_EX1':
    ...
    exec_data, exec_detection, exec_tracking, exec_visualization = [], [], [], ['show_range_image']
    configs_det = det.load_configs(model_name='darknet')
if exercise == 'ID_S1_EX2':
    data_filename = 'training_segment-10963653239323173269_1924_000_1944_000_with_camera_labels.tfrecord' # Sequence 3
    show_only_frames = [0, 200]  # show only frames in interval for debugging
    exec_data, exec_detection, exec_tracking, exec_visualization = [], [], [], ['show_pcl']
    configs_det = det.load_configs(model_name='darknet')
if exercise == 'ID_S2_EX1' or exercise == 'ID_S2_EX2' or exercise == 'ID_S2_EX3':
    data_filename = 'training_segment-1005081002024129653_5313_150_5333_150_with_camera_labels.tfrecord'  # Sequence 1
    show_only_frames = [0, 1]  # show only frames in interval for debugging
    exec_data, exec_detection, exec_tracking, exec_visualization = ['pcl_from_rangeimage'], ['bev_from_pcl'], [], []
    configs_det = det.load_configs(model_name='darknet')
if exercise == 'ID_S3_EX1' or exercise == 'ID_S3_EX2':
    data_filename = 'training_segment-1005081002024129653_5313_150_5333_150_with_camera_labels.tfrecord'  # Sequence 1
    show_only_frames = [50, 51]  # show only frames in interval for debugging
    exec_data, exec_detection, exec_tracking, exec_visualization = ['pcl_from_rangeimage', 'load_image'], \
                                                                   ['bev_from_pcl', 'detect_objects'], \
                                                                   [], \
                                                                   ['show_objects_in_bev_labels_in_camera']
    configs_det = det.load_configs(model_name="fpn_resnet")
if exercise == 'ID_S4_EX1' or exercise == 'ID_S4_EX2':
    data_filename = 'training_segment-1005081002024129653_5313_150_5333_150_with_camera_labels.tfrecord'  # Sequence 1
    show_only_frames = [50, 51]  # show only frames in interval for debugging
...
if exercise == 'final_step1':
    ...
    configs_det.lim_y = [-5, 10] # limit y-range and remove other targets left and right of our target
if exercise == 'final_step2':
    data_filename = 'training_segment-10072231702153043603_5725_000_5745_000_with_camera_labels.tfrecord'  # Sequence 2
...
if exercise == 'final_step3' or exercise == 'final_step4':
    data_filename = 'training_segment-1005081002024129653_5313_150_5333_150_with_camera_labels.tfrecord'  # Sequence 1
    show_only_frames = [0, 200]  # show only frames in interval for debugging
    exec_data, exec_detection, exec_tracking, exec_visualization = [], \
                                                                   [], \
...
# set to True: use groundtruth labels as objects (False: use model-based detection)
configs_det.use_labels_as_objects = True
...
while True:
    try:
        ...
        if 'show_range_image' in exec_list:
            ...
            cv2.imwrite(f'custom_plots/stacked_range_image{data_filename}.jpg', img_range)
